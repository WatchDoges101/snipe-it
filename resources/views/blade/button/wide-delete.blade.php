...
@can('delete', $item)
    ...
    @if ((method_exists($item, 'isDeletable')) && ($item->deleted_at==''))
        @if (!$item->isDeletable())
            <button class="btn btn-block btn-sm btn-danger btn-social hidden-print disabled"{!! $tooltip ? ' data-tooltip="true" data-placement="top" data-title="'.e(trans('general.cannot_be_deleted')).'"' : '' !!}>
                <x-icon type="delete" />
